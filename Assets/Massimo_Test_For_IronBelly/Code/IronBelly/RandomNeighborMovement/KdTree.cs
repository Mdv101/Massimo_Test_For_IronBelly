...
public class KdTree<T> : IEnumerable<T>, IEnumerable where T : Component
{
    ...
    public float SearchLength { set; get; } = 99;
    ...
    public T FindClosest(Vector3 position, T ExcludedItem = null)
    {
        
        return _findClosest(position, null, ExcludedItem);
    }
    ...
    public IEnumerable<T> FindClose(Vector3 position, T ExcludedItem = null)
    {
        var output = new List<T>();
        _findClosest(position, output, ExcludedItem);
        return output;
    }

    protected T _findClosest(Vector3 position, List<T> traversed = null, T ExcludedItem = null)
    {
        if (_root == null)
            return null;
        ...
        AverageSearchLength = (SearchLength * AverageSearchLength + openCur) / 100f;

        if (nearest != null)
        {
            AverageSearchDeep = (SearchLength * AverageSearchDeep + nearest.level) / 100f;   
